...
// Watch for route changes to reload content when navigating between blog posts
watch(() => route.params.id, async (newId, oldId) => {
  if (newId && newId !== oldId) {
    ...
    // Reload comments
    if (newId) {
      await fetchComments(newId as string);
    }

    // Re-observe fade elements after content updates
    setTimeout(() => {
      observeFadeElements();
    }, 100);
  }
